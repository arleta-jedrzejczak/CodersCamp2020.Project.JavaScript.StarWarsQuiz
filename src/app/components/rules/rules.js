class Rules {
  renderHeader(mode) {
    ...
    cont.style =
      'padding: 16px; font-size: 24px; font-weight: bold; box-shadow: 0px 4px 4px rgba(0, 0, 0, 0.25), 4px 4px 40px rgba(255, 0, 0, 0.9), inset 4px 5px 4px rgba(0, 0, 0, 0.5); border-radius: 8px;';
  ...
  renderDescription(mode) {
    if (mode.includes('intro')) {
      ...
      const descriptionContent = document.createTextNode(
        `You have one minute (1m) to answer as many questions as possible. During the game on each question you need to select what ${
          mode == 'characters-intro'
            ? 'character'
            : mode == 'vehicles-intro'
            ? 'vehicle'
            : mode == 'starships-intro'
            ? 'starship'
            : 'thing'
        } from Star Wars is showed on the left image from available options.`,
      );
      ...
      descriptionTitleImg.setAttribute(
        'src',
        'https://images.vexels.com/media/users/3/143402/isolated/preview/afbbf15d5e82a1c4fb5a55c4eacf3003-graduation-cap-icon-by-vexels.png',
      );

      descriptionTitleImg.style =
        'width: 1.6em; height: 1.5em; margin-right: 1em;';
      descriptionTitleContainer.style =
      ...
      cont.style =
        'padding: 12px; box-shadow: 0px 4px 4px rgba(0, 0, 0, 0.25); border-radius: 8px; font-size: 20px;';
